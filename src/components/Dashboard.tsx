...
export default function Dashboard({
  ...
  // Active waveform state for header display
  const [activeWaveform, setActiveWaveform] = useState<ActiveWaveform | null>(null);
  ...
  useEffect(() => {
    ...
    const unlistenComplete = listen<string>("playback-complete", (event) => {
      ...
      if (DEBUG) console.log(`[COMPLETE] Playback complete: ${completedPlaybackId}`);
      ...
      if (!soundId) {
        if (DEBUG) console.log(`[WARN] Playback ${completedPlaybackId} not found in ref`);
        return;
      }
      
      if (DEBUG) console.log(`[CLEANUP] Cleaning up playback for sound: ${soundId}`);
      ...
      setActiveWaveform(prev => {
        if (prev?.soundId === soundId) {
          ...
          const otherPlayingSounds = Array.from(playingSoundsRef.current.keys()).filter(id => id !== soundId);
          if (otherPlayingSounds.length > 0) {
            // Switch to the last remaining sound (newest)
            const nextSoundId = otherPlayingSounds[otherPlayingSounds.length - 1];
            const nextSound = soundLibrary.sounds.find(s => s.id === nextSoundId);
            if (nextSound) {
              return {
    ...
    const unlistenProgress = listen<PlaybackProgress>("playback-progress", (event) => {
      ...
      for (const [soundId, pid] of playingSoundsRef.current.entries()) {
        if (pid === playback_id) {
          setActiveWaveform(prev => {
            if (prev?.soundId === soundId) {
              return { ...prev, currentTimeMs: elapsed_ms, durationMs: total_ms };
            }
            return prev;
          });
          break;
    ...
    const unlistenFileDrop = listen<{ paths: string[] }>('tauri://drag-drop', (event) => {
      ...
      if (audioFiles.length > 0) {
        if (DEBUG) console.log(`[DROP] Dropped ${audioFiles.length} audio file(s):`, audioFiles);
        ...
        if (audioFiles.length === 1) {
          ...
          setEditingSound(null);
          setIsModalOpen(true);
          showToast(`Adding ${audioFiles.length} sounds (1/${audioFiles.length})`);
        }
      } else {
  ...
  const categorySounds = soundLibrary.sounds.filter(s => s.category_id === selectedCategoryId);
  ...
  const playSound = useCallback(async (sound: Sound) => {
    ...
    if (shouldRestart && currentPlaybackId) {
      if (DEBUG) console.log(`[RESTART] Restarting: ${sound.name} (${currentPlaybackId})`);
      
      try {
        await invoke("stop_playback", { playbackId: currentPlaybackId });
        if (DEBUG) console.log(`[STOP] Stopped playback: ${currentPlaybackId}`);
    ...
    try {
      ...
      if (DEBUG) console.log(`[PLAY] Started playback: ${playbackId} for ${sound.name}`);
  ...
  const handleToggleFavorite = async (sound: Sound) => {
    try {
      ...
      await refreshSounds();
      showToast(sound.is_favorite ? `Removed from favorites: ${sound.name}` : `Added to favorites: ${sound.name}`);
    } catch (error) {
  ...
  const handleTrimSave = useCallback(async (_trimStartMs: number | null, _trimEndMs: number | null) => {
    await refreshSounds();
        ...
        <div className="flex items-center justify-between gap-4 h-10">
          ...
          <div className="flex items-center gap-3 flex-shrink-0 h-full">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className={`text-xs w-8 ${volume >= 0.75 ? "text-red-400" : "text-discord-text"}`}>
        ...
        <div className="flex-1 overflow-auto">
          {filteredSounds.length === 0 ? (
            ...
            <div className="space-y-6">
              ...
              {/* Divider */}
              {!showFavoritesOnly && favoriteSounds.length > 0 && regularSounds.length > 0 && (
